...
module.exports = function(express, io, mongo) {
    ...
    express.get("/v2/hello", async(request, response) => {
        response.sendStatus(500);
    })
    
    express.get("/v2/create_code", async (request, response) => {
        try {
            ...
            let code = utils.createRandomCode(user_id, "2");
            ...
            response.json({"pay_code": code, "expires_date": date.toISOString(), "user_id": user_id})
    ...
    express.get("/v2/make_payment", async (request, response) => {
        try {
            let code = request.query.pay_code
            ...
            if (!activeCode) {
                response.json(
                    {"result": false, "msg": "Invalid pay code"}
                )
                return
            }
            ...
            if (!activeCode.is_active) {
                response.json(
                    {"result": false, "msg": "Code " + code + " is invalid"}
                )
                return
            }
            ...
            if (activeCode.is_paid) {
                response.json(
                    {"result": false, "msg": "Code " + code + " is paid"}
                )
                return
            }
            ...
            response.json(
                {"result": true, "payment_id": inserted.insertedId, "pay_code": code},
            );
